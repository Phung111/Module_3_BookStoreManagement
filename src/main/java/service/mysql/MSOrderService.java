...
public class MSOrderService extends DBContext implements IOrderService {
    private static final String ADD_ORDER_ITEM = "INSERT INTO `module_3_bookstoremanagement`.`order_item` (`id_order`, `id_product`, `quantity`) VALUES (?, ?, ?)";
    String ADD_ORDER = "INSERT INTO `module_3_bookstoremanagement`.`order` (`address`, `phone`, `name`, `total`) VALUES (?, ?, ?, ?)";
    @Override
...
//        VALUES ('28 NTP', '3456789', 'Quang', '12');\n";
        try {
            ...
            for (int i = 0; i < order.getOrderItems().size(); i++) {
                ...
                ps.setLong(2, order.getOrderItems().get(i).getIdBook());
                ps.setLong(3, order.getOrderItems().get(i).getQuantiy());
                ps.executeUpdate();

                connection.prepareStatement("INSERT INTO `module_3_bookstoremanagement`.`order_item` (`id_order`, `id_product`, `quantity`) VALUES ('21', '3', '1')").executeUpdate();
    ...
    public void saveOrderBySPWithParameter(Order order) {
        ...
        System.out.println(jsonInString);
        try {
            CallableStatement callableStatement = connection.prepareCall("call module_3_bookstoremanagement.spSaveOrderWithParameter(?, ?)");
            callableStatement.setObject(1, jsonInString);
